...
fn main() {
    ...
    let target = &args[1];
    match ps_utils::get_target(target.as_str()).expect("fail to call ps or pgrep") {
        Some(p) => {
            p.print();
            for sub_p in ps_utils::get_child_processes(p.pid)
                .expect("something wrong in get_child_processes")
            {
                sub_p.print();
            }
        }
