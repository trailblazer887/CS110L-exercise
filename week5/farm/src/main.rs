...
fn main() {
    ...
    for _ in 0..num_threads {
        ...
        let handle = thread::spawn(move || {
            while let Some(num) = nums.lock().expect("some threads have panic.").pop_front() {
                // lock has been released
                println!("Thread {:?} processing {}", thread::current().id(), num);
                factor_number(num);
            }
        });
        handles.push(handle);
    ...
    for handle in handles {
        handle.join().expect("some threads has panic");
    }
